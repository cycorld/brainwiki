...
@graph_render = ->
	...
	d3.json '/nuerons.json', (error, graph) ->
		...
		click = (d) ->
			...
			return
		mouseenter = ->
			d3.select(this).transition().duration(750).attr('r', 16).style 'fill', 'lightsteelblue'
			...
			return
		mouseleave = ->
			d3.select(this).transition().duration(750).attr('r', 6).style 'fill', (d) ->
				color d.user_id
			d3.select(this).select('text').transition().duration(750).attr('x', 12).style('stroke', 'none').style('fill', 'black').style('stroke', 'none').style 'font', '10px sans-serif'
			return
		...
		node = svg.append('svg:g').selectAll('.node').data(graph.nuerons).enter().append('circle').attr('class', 'node').attr('r', 6).style('fill', (d) ->
			color d.user_id
		).on('click', click).on('mouseenter', mouseenter).on('mouseleave', mouseleave).call(force.drag)
